...
| reporter_username?<sup>1</sup> | string | The reporter's own username (1-32 characters) |
...
| reported_user_username?<sup>2</sup> | string | The message author's username (1-32 characters) |
...
<sup>2</sup> When supplied it resolves to exactly the account that authored the linked message. A mismatch returns 400 `INVALID_DSA_REPORT_TARGET`, and a username that resolves to no account returns 404 `UNKNOWN_USER`
...
| username?<sup>1</sup> | string | The reported user's username (1-32 characters) |

<sup>1</sup> Supply at least one of the two. A body that supplies neither returns 400 `INVALID_FORM_BODY`, and an ID or username that resolves to no account returns 404 `UNKNOWN_USER`
...
| 400 | [error response](/http-api/#error-response) | The ticket is unknown or expired and the request returns `INVALID_DSA_TICKET`, or a supplied username or invite code is malformed or does not match the reported message author, user, or guild and the request returns `INVALID_DSA_REPORT_TARGET` |
...
Every notice also records `additional_info`, the reporter's legal name, country of residence, and the verified address behind the ticket. A notice has no account ID even when the request presented a credential, and it discards `reporter_username`.
