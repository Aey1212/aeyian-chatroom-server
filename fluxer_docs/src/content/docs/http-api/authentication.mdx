...
:::note[The successful result has no type marker]
...
| username<sup>2</sup> | string | The account username and login ID (1-32 characters, letters, digits, and underscores) |
...
<sup>1</sup> Omitting the email address creates an account without one. It signs in with its username, and its password can be reset only after an admin opens a reset for it, see [reset an opened password](#reset-an-opened-password). An address whose domain has no usable DNS records, or whose top-level domain is blocked by account policy, returns the field code `INVALID_EMAIL_ADDRESS`, and an address already in use returns `EMAIL_ALREADY_IN_USE`

<sup>2</sup> The username is unique on the instance ignoring letter case. A username in use, locked after a deleted account or a rename, or held by a pending rename request returns the field code `USERNAME_ALREADY_TAKEN`
...
Registration returns 403 `REGISTRATION_CLOSED` when the instance is closed and no valid registration URL was supplied. A username or display name containing a blocked substring returns 403 `CONTENT_BLOCKED`.
...
Validates a username or email address with a password. Authentication is not required. Returns an [authentication token response](#authentication-token-response) when no second factor and no IP approval are outstanding, and an [MFA challenge response](#mfa-challenge-response) when the account has a second factor.

Login verifies CAPTCHA when CAPTCHA is enabled. It also permits 10 attempts per client IP address in 30 minutes, keyed by the exact IPv4 address or the IPv6 /64 network, and 5 attempts per login value in 15 minutes, ignoring letter case. Every admitted attempt consumes both allowances, whether or not the credentials turn out to be correct, and a successful login clears neither.
...
| Field | Type | Description |
| --- | --- | --- |
| login<sup>1</sup> | string | The account username or email address (1-254 characters) |
| password | string | The account password (8-256 characters) |
| invite_code?<sup>2</sup> | ?string | The invite accepted immediately after a successful login (0-256 characters) |

<sup>1</sup> A value containing `@` is looked up as an email address and any other value as a username, both ignoring letter case

<sup>2</sup> The invite is accepted only on a login that completes without a second factor, and a failure to accept it does not fail the login. A login held for IP authorisation discards the invite

:::note[Unknown accounts and unknown passwords are indistinguishable]
An unknown username or email address and an incorrect password all return the same paired field codes `INVALID_EMAIL_OR_PASSWORD` on `login` and `password`.
:::
...
When the instance has disabled new-IP authorisation or sends no email, or the account has no email address, Fluxer authorises the client IP address silently and the login continues. An account that already has a second factor never enters IP authorisation.
...
| 400 | [error response](/http-api/#error-response) | The body, CAPTCHA, login, or password is invalid |
...
| 429 | [rate limit response](/topics/rate-limits/#rate-limit-response-object) | A route, global, login, or IP bucket denies the request |
| 500 | [error response](/http-api/#error-response) | Unexpected internal failure occurs |
| 503 | [error response](/http-api/#error-response) | The instance is at its in-flight request ceiling |
...
## Reset an opened password

<RouteHeader method="POST" path="/v1/auth/password-reset" unauthenticated />

Replaces the password of an account an admin has opened a password reset for, then issues a new session or an MFA challenge. No email or token is involved: the admin opens the reset with [set password reset mode](/admin-api/users/#set-password-reset-mode), and the reset closes when it is used. Returns an [authentication token response](#authentication-token-response) when the account has no second factor, and an [MFA challenge response](#mfa-challenge-response) when it has one.
...
| --- | --- | --- |
| login<sup>1</sup> | string | The account username or email address (1-254 characters) |
| password<sup>2</sup> | string | The replacement password (8-256 characters) |

<sup>1</sup> A value containing `@` is looked up as an email address and any other value as a username, both ignoring letter case

<sup>2</sup> The password is checked against the public breached-password corpus, and a match returns the field code `PASSWORD_IS_TOO_COMMON`

An unknown account, a deleted account, and an account with no open reset all return the field code `INVALID_OR_EXPIRED_RESET_TOKEN` on `login`. A bot account returns 403 `BOT_USER_AUTH_ENDPOINT_ACCESS_DENIED`, and a live suspension returns its 403.
...
| --- | --- | --- |
| 200 | [authentication token response](#authentication-token-response) \| [MFA challenge response](#mfa-challenge-response) | The password was replaced |
| 400 | [error response](/http-api/#error-response) | The body is invalid, no reset is open, or the password is too common |
| 403 | [error response](/http-api/#error-response) | A bot account or a suspension rejects the mutation |

### Side effects

The operation replaces the password, records the change time, closes the open reset, and terminates every authentication session on the account. An account with no second factor then receives one new session and its token. An account with a second factor receives a five-minute MFA ticket instead.

### Rate limit

10 requests per minute, on the `auth:reset` bucket, shared with [reset a password](#reset-a-password).
...
The operation does no availability check and holds nothing. Registration returns `USERNAME_ALREADY_TAKEN` when the suggested username is taken by then.
:::
