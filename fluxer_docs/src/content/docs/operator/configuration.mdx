...
| --- | --- |
| FLUXER_REGISTRY_OWNER | `aey1212` |
| FLUXER_REGISTRY | `ghcr.io/${FLUXER_REGISTRY_OWNER}` |
| FLUXER_IMAGE_TAG | `v1` |

`FLUXER_REGISTRY_OWNER` is the owner segment of the image names and falls back to `aey1212`, the owner of this fork's images. `FLUXER_REGISTRY` is the registry images are pulled from and falls back to `ghcr.io/` followed by the owner. `FLUXER_IMAGE_TAG` is the tag on every Fluxer image and falls back to `v1`.
