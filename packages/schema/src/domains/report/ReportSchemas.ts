...
import {EmailType, UsernameLookupType} from '@fluxer/schema/src/primitives/UserValidators';
import {z} from 'zod';
...
const DsaReportBase = z.object({
	...
	reporter_username: z.optional(UsernameLookupType).describe('Username of the reporter if they have an account'),
});
const DsaReportMessageRequest = DsaReportBase.extend({
	...
	reported_user_username: z.optional(UsernameLookupType).describe('Username of the user who sent the message'),
});
...
const DsaReportUserRequest = DsaReportBase.extend({
	...
	username: z.optional(UsernameLookupType).describe('Username of the user being reported'),
}).superRefine((value, ctx) => {
	if (!value.user_id && !value.username) {
		ctx.addIssue({
			code: 'custom',
			message: 'Either user_id or username must be provided for user reports',
			path: [],
		});
	}
});
